...
SELECT
  ...
  age - AVG(age) OVER (PARTITION BY department) as age_diff_from_avg
FROM people
...
INSERT INTO people (name, age, email, department)
VALUES ('Kevin', 28, 'kevin@example.com', 'Engineering');

-- Example 13: Verify the insert
SELECT * FROM people WHERE name = 'Kevin';

-- Example 14: Update data
UPDATE products SET stock = stock + 50 WHERE category = 'Electronics';

-- Example 15: Verify the update
SELECT name, category, stock FROM products WHERE category = 'Electronics';
